...
logger = logging.getLogger("Hz.Main")
...
async def main() -> None:
    ...
    async with bot:
        ...
        async def on_ready() -> None:
            ...
            if not getattr(bot, "_hz_synced", False):
                bot._hz_synced = True  # type: ignore[attr-defined]
                dev = (bot.config.get("DEV_GUILD") or "").strip()  # type: ignore[union-attr]
                if dev.isdigit():
                    try:
                        gobj = discord.Object(id=int(dev))
                        bot.tree.copy_global_to(guild=gobj)
                        synced = await bot.tree.sync(guild=gobj)
                        logger.info(
                            "Slash commands synchronisées sur DEV_GUILD (%s) : %s",
                            dev,
                            ", ".join(c.name for c in synced) or "—",
                        )
                    except discord.HTTPException as exc:
                        logger.error("Échec sync DEV_GUILD : %s", exc)
